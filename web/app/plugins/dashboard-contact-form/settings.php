	...
	function cdw_widget_options(){
		...
		$cdw_pass = get_option('cdw_password', $empty_pass );
		 ...
		 if (isset($_POST['cdw_new_pass1']) ){
				 if($cdw_pass == md5(filter_input(INPUT_POST,'cdw_old_pass')) || $cdw_pass == $empty_pass || wp_verify_nonce( filter_input(INPUT_POST,'cdw_new_pass'), 'cdw_reset_password' ))  
				 {
					 if(filter_input(INPUT_POST,'cdw_new_pass1') == filter_input(INPUT_POST,'cdw_new_pass2')){
						 update_option('cdw_password',md5(filter_input(INPUT_POST,'cdw_new_pass1')));
						 $cdw_pass = get_option('cdw_password');
						 $_SESSION['cdw_verified'] = md5(filter_input(INPUT_POST,'cdw_new_pass1'));
		...
		if(filter_input(INPUT_GET,'action') === 'forgotten_password' )
		{
			if (cdw_reset_mail())
			{
		...
		if(filter_input(INPUT_GET,'action') === 'reset_password')
		{
			if(cdw_verify_nonce(filter_input(INPUT_GET,'cdw_key'),"reset_password"))
			{
				?>
		...
		if ($cdw_pass ==  md5(filter_input(INPUT_POST,'cdw_pass')) || $cdw_pass == $empty_pass || @$_SESSION['cdw_verified'] == $cdw_pass  )
		{
			if (!isset($_SESSION['cdw_verified']))
			{	
				isset($_POST['cdw_pass']) ? $_SESSION['cdw_verified'] = md5(filter_input(INPUT_POST,'cdw_pass')) : $_SESSION['cdw_verified'] = $empty_pass;
			}
		...
		if (isset($_POST['cdw_pass']) && md5(filter_input(INPUT_POST,'cdw_pass')) != filter_input(INPUT_SESSION,'cdw_verified'))
		{
			echo '<div id="message" class="error"><p><strong>'.__('Error: invalid password','cdw_widget').'</strong></p></div>';
		}
		if (filter_input(INPUT_GET,'action') === 'cdw_logout' && isset($_SESSION['cdw_verified'])) 
		{ 
...
if (!function_exists('cdw_widget_options_form')) {
	function cdw_widget_options_form() {       
		...
		if(isset($_POST['action'])) {
		 ...
		 $cdw_data = array( 
                                    ...
                                    'target_mail' 	=>  is_email(filter_input(INPUT_POST,'target_mail')) ? sanitize_email(filter_input(INPUT_POST,'target_mail')) : $cdw_old['target_mail'],  
                                    'hide_target'       => isset($_POST['hide_target']) ? true : false,
                                    'cc_mail' 				=>  cdw_are_emails($_POST['cc_mail']) ? $_POST['cc_mail'] : $cdw_old['cc_mail'],
                                    'form_title' 			=>  wp_kses_post(filter_input(INPUT_POST,'form_title')) ,
                                    'form_message' 			=>  wp_kses_post(filter_input(INPUT_POST,'form_message')) ,
                                    'placeholder_title' 	=>  wp_kses_post(filter_input(INPUT_POST,'placeholder_title')) ,
                                    'placeholder_message' 	=>  wp_kses_post( filter_input(INPUT_POST,'placeholder_message') ),
                ...
                if(is_multisite() && is_super_admin() && basename(filter_input(INPUT_SERVER,'PHP_SELF')) == 'options-general.php' ){
                        ...
                        if($cdw_blogs)
                        {
                            foreach($cdw_blogs as $cdw_blog){
                                update_blog_option($cdw_blog, 'cdw_dashboard_text', serialize($cdw_data));
                                $blogs_msg .= "<br />".sprintf(__('Blog #%s succesfully updated','cdw_widget'),$cdw_blog)."\n";
                            }
                        }
                        update_blog_option(1,'cdw_blogs',$cdw_blogs);
                        $cdw_enable_blogs = isset($_POST['blog_enable_id']) ? array_values($_POST['blog_enable_id']) : array();
                        update_blog_option(1,'cdw_enable_blogs',$cdw_enable_blogs);
                 }
			if (!is_email(filter_input(INPUT_POST,'target_mail'))) { echo '<div id="message" class="error is-dismissible"><p><strong>'; 
			_e('Email has invalid format, not updated. Other settings saved. (You may have only one e-mail in this field).','cdw_widget');}
				else { echo '<div id="message" class="updated is-dismissible"><p><strong>'.__('All settings successfully saved.','cdw_widget').$blogs_msg;}
			echo '</strong></p></div>';
			update_option('cdw_dashboard_text',serialize($cdw_data));
			unset($cdw_data);
		 }
		 else echo '<div id="message" class="error is-dismissible"><p><strong>'.__('ERROR: No settings saved, try again.','cdw_widget').'</strong></p></div>';
		 }
		 /* Multisite enabled check */
		 if(is_multisite()){
                $cdw_enable_blogs = maybe_unserialize(get_blog_option(1,'cdw_enable_blogs',array()));
				//var_dump($cdw_enable_blogs);
				/* Disable edit if setttings override is disabled by superadmin except the superadmins */
                if(is_multisite() && !in_array(get_current_blog_id(),$cdw_enable_blogs) && !is_super_admin() )  {
                    

                    echo '<div id="message" class="error is-dismissible"><p><strong>'.__('Edit of this blog is disabled by superadmin.','cdw_widget').'</strong></p></div>';
                    return false;
                }
				/* Warning message for superadmins */
				if(is_multisite() && !in_array(get_current_blog_id(),$cdw_enable_blogs) && is_super_admin() )  {
                    
                    echo '<div id="message" class="error is-dismissible"><p><strong>'.__('Warning, edit of this blog is disabled except to superadmins.','cdw_widget').'</strong></p></div>';
                }
            }
            else {
                $cdw_enable_blogs = array();
            } 
	...
	//if(isset($_POST) && WP_DEBUG && WP_DEBUG_DISPLAY) echo "POST: <pre>".print_r($_POST,true)."</pre>";
		...
		<form method="post" action="<?php echo admin_url('options-general.php?page=cdw_widget_options'); ?>">
		<table><tbody>
		<tr><td><label for="title"><?php _e('Widget title:','cdw_widget'); ?></label></td><td><input type="text" class="style-input" name="title" value="<?php echo stripslashes(htmlspecialchars($cdw_data['title'],ENT_QUOTES,$encoding)); ?>" />
		...
		<tr><td><label for="message"><?php _e('Widget text:','cdw_widget'); ?></label></td><td><?php  wp_editor( stripslashes($cdw_data['message']), 'message_editor', array('textarea_name' => 'message', 'textarea_rows' => '20','tinymce' => true , 'drag_drop_upload' => true) ); ?> </td></tr>
		...
		foreach($rolles as $key=>$rolename)
		{
		/*
                 *  enable for roles
                 */
		$readonly = $rolename == "administrator"  ? "readonly=\"readonly\" onclick=\"return false\" " :  "";
		$checked = $rolename == "administrator" ? "checked=\"checked\"" : cdw_checked($key,$cdw_data['roles']);
		echo "<tr><td><input type=\"checkbox\" name=\"cdw_role[]\" class=\"cdw_role\" ".$readonly."id=\"role_".$key."\" value=\"".$key."\" ".$checked."><label class=\"cdw_role_label\" for=\"role_".$key."\">".translate_user_role($rolename)."</label></td></tr>\n";
		...
		<?php 
                /*
                 *  Enable contact form for users with roles: ?>
                 */ ?>
		<table><tbody>
		<tr><th><?php _e('Enable contact form for roles:','cdw_widget'); ?></th></tr> <?php  echo "\n";
		foreach($rolles as $key=>$rolename)
		{
			//$readonly_cf = !in_array($key, $cdw_data['roles']) && $key != "administrator"  ?  "readonly=\"readonly\" onclick=\"return false\" disabled=\"disabled\" " :  "";
			$readonly_cf = "";
			$checked_cf = cdw_checked($key,$cdw_data['roles_cf']);
			echo "<tr><td><input type=\"checkbox\" name=\"cdw_role_cf[]\" class=\"cdw_role\" ".$readonly_cf."id=\"role_".$key."_cf\" value=\"".$key."\" ".$checked_cf."><label class=\"cdw_role_label\" for=\"role_".$key."_cf\">".translate_user_role($rolename)."</label></td></tr>\n";
		}
		?></tbody></table>
		...
		<?php 
		/* Enable for multisite blogs: */
           
                if(is_multisite() && is_super_admin() && basename(filter_input(INPUT_SERVER,'PHP_SELF')) == 'options-general.php' ){   
                    $cdw_blogs = get_blog_option(1,'cdw_blogs',array());
                    $blogs = wp_get_sites();
                    ?> <h3><?php _e('Copy settings on blogs:','cdw_widget'); ?></h3>
                    <table><thead><tr><th><input id="input_blog" type="checkbox" /></th><th><?php _e('ID','cdw_widget'); ?></th><th><?php _e('Blog url:','cdw_widget'); ?></th></tr></thead>
                    <tbody>
                        <?php foreach($blogs as $blog)
                        {
                            
                               $checked = in_array($blog['blog_id'],$cdw_blogs,true) ? "checked=\"checked\"" : "";
							    $current = BLOG_ID_CURRENT_SITE == $blog['blog_id'] ? '<em> <strong>'.__('(Current)','cdw_widget').'</strong></em>' : '';
								$disabled = BLOG_ID_CURRENT_SITE == $blog['blog_id'] ? 'disabled="disabled"' : '';
                        echo "<tr><td><input class=\"input_blog\" type=\"checkbox\" ".$checked." ".$disabled." name=\"blog_id[]\" value=\"".$blog['blog_id']."\" /></td><td><strong>".$blog['blog_id'].":</strong></td>"
                             ."<td><em>".$blog['domain'].$blog['path']."</em>".$current."</td></tr>\n"; 
                        }
                            ?>
                    </tbody> 
                    </table><br />
                   ...
                   <?php /* Moved to top */
                    $cdw_enable_blogs = get_blog_option(1,'cdw_enable_blogs',array());
                    
                    ?> <h3><?php _e('Enable override settings on blogs:','cdw_widget'); ?></h3>
					
                    <table><thead><tr><th><input id="input_enable_blog" type="checkbox" /></th><th><?php _e('ID','cdw_widget'); ?></th><th><?php _e('Blog url:','cdw_widget'); ?></th></tr></thead>
                    <tbody>
                        <?php foreach($blogs as $blog)
                        {
                           
                               $checked = in_array($blog['blog_id'],$cdw_enable_blogs,true) ? "checked=\"checked\"" : "";
							   $current = BLOG_ID_CURRENT_SITE == $blog['blog_id'] ? '<em> <strong>'.__('(Current)','cdw_widget').'</strong></em>' : '';
                        echo "<tr><td><input class=\"input_enable_blog\" type=\"checkbox\" ".$checked." name=\"blog_enable_id[]\" value=\"".$blog['blog_id']."\" /></td><td><strong>".$blog['blog_id'].":</strong></td>"
                             ."<td><em>".$blog['domain'].$blog['path']."</em>".$current."</td></tr>\n";
	...
	<img type="image" src="https://www.paypalobjects.com/en_US/i/btn/btn_donateCC_LG.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!" />
        ...
        <p><?php _e('You may also donate any amount of money. (Thank you!)','cdw_widget'); ?></p>
        <a target="_blank" href="https://www.paypal.com/cgi-bin/webscr?cmd=_donations&amp;business=ripasu%40volny%2ecz&amp;item_name=Dashbopard%20Contact%20Form%20Donation&amp;item_number=Support%20Open%20Source&amp;no_shipping=0&amp;no_note=1&amp;tax=0&amp;currency_code=USD&amp;lc=US&amp;bn=PP%2dDonationsBF&amp;charset=UTF%2d8">
	<img type="image" src="https://www.paypalobjects.com/en_US/i/btn/btn_donateCC_LG.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!" />
	</a>
...
if (!function_exists('cdw_are_emails')) {	
	function cdw_are_emails($string){
		...
		$emails = explode(",",str_replace(";",",",$string));
...
if (!function_exists('cdw_verify_nonce')) {
	function cdw_verify_nonce( $nonce, $action = -1 ) {
		...
		$expected_12 = substr( wp_hash( $i . '|' . $action . '|' . $admin_email . '|' . $token, 'nonce'), -12, 10 );
		if ( hash_equals( $expected_12, $nonce ) ) {
			return 1;
		}

		// Nonce generated 12-24 hours ago
		$expected_24 = substr( wp_hash( ( $i - 1 ) . '|' . $action . '|' . $admin_email . '|' . $token, 'nonce' ), -12, 10 );
		if ( hash_equals( $expected_24, $nonce ) ) {
			return 2;
		}
...
add_action( 'admin_enqueue_scripts', 'cdw_admin_style' );
if( !function_exists('cdw_admin_style')) {
    function cdw_admin_style()
    {
        global $pagenow;
        if($pagenow == 'cdw_widget_options' || true){
            wp_enqueue_script( 'cdw_admin_script', plugin_dir_url( __FILE__ ) . 'js/admin.js' );
        }
    }
}