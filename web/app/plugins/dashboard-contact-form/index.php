...
function cdw_full_install($networkwide) {
    ...
    if(is_multisite()) {
        add_option('cdw_enable_blogs',cdw_all_blogs());
        add_option('cdw_blogs',cdw_all_blogs());
    }
}
/**
...
function cdw_full_uninstall($networkwide) {
    ...
    /* Delete these options anyway */
    delete_option('cdw_enable_blogs');
    delete_option('cdw_blogs');
...
if (!function_exists('cdw_init')) {
	function cdw_init() { 
		...
		$session_id = 'CDW'.md5($current_user->user_login);
 ...
 * @return void
 */
add_action( 'wpmu_new_blog', 'cdw_new_blog', 10, 1);        
 
function cdw_new_blog($blog_id ) {
    global $wpdb;
	$old_blog = $wpdb->blogid;
...
if (!function_exists('function cdw_default_data')) {
	...
	load_plugin_textdomain( 'cdw_widget', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
	 ...
	 'roles'				=>  array_keys(cdw_all_roles()),
	 'roles_cf'				=>  array_keys(cdw_all_roles())
	 ));
	}
}
/*
...
/* deprecated */
if (!function_exists('cdw_all_blogs')){
    
    function cdw_all_blogs() {
        if (is_multisite()){
            ...
            foreach($blogs as $blog)
            {
                    ...
                    $cdw_enable_blogs[] = $blog['blog_id'];
            }
            return $cdw_enable_blogs;
        }
        else {
            return array();
